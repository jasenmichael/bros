...
const { conversations, refreshAgentRecents } = useAgentRecents()
...
async function removeAgent(id: string) {
  ...
  if (route.path === `/agent/${id}`) await router.replace('/agent')
  await refreshAgentRecents()
}
...
<template>
  ...
  <div v-if="showList" class="bros-recents">
    <div class="bros-nav-panel__divider" role="separator" aria-label="Previous agents" />
    <button
      type="button"
    ...
    >
      <span class="bros-nav-label">Previous agents</span>
      <UIcon
