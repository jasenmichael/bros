...
- Chat/Models: Models is Ollama (sidecar then host), Popular services (12 always-listed OpenAI-compat cards), then Custom providers. Rows show host:port (`127.0.0.1:11435` sidecar publish, not Docker DNS) or the API host for popular/custom, with copy + open-in-new-tab. Selecting a different Ollama card expands pull + installed models on that card; the chevron (or a second click on the open card) collapses the panel. Ollama, Popular, and Custom lists stay full BrosPageShell width whether a card is open or collapsed. Each row has a settings cog (UModal); GPU only in the sidecar modal. Popular cog is key + models (no Delete). Host card stays listed when the daemon is down. Unconfigured popular cards stay listed. Chat is provider dropdown then model dropdown (order: sidecar, host, popular, custom). Heading is Popular services, never “Paid providers”.
