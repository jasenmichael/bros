...
Sidecar Ollama is the only must-run core sidecar. Whisper is also core and stays stopped until Settings enables it. Bros starts Ollama on app start and keeps it up. Sidecars / Status / Home have no Start, Stop, Restart, or Autostart. There is no disable env. `POST /api/sidecars/ollama/start`, `…/stop`, and `…/restart` return 400. GPU apply still recreates the container internally.
...
- `$BROS_HOST_DATA_DIR/ollama/root/.ollama` → `/root/.ollama`
- `$BROS_HOST_DATA_DIR/ollama/root/.config/ollama` → `/root/.config/ollama`
- `$BROS_HOST_DATA_DIR/ollama/bros-model` → `/bros-model:ro` (packaged specialist)
...
On first sidecar up, Bros copies the packaged GGUF tree onto `$BROS_HOST_DATA_DIR/ollama/bros-model` and runs `bash /bros-model/scripts/install-ollama.sh` inside `bros-sc-ollama` (`ollama create bros`). Used for Chat auto-title. **Not listed** in Chat or Providers. Pull/delete of the reserved name returns 400. Source: [jasenmichael/bros-model](https://github.com/jasenmichael/bros-model).
