...
- Sidecars stay **sidecars** (`/sidecars`, `/api/sidecars`, `sidecars/`, `sidecar.yml`). Layout: `sidecars/core/` (Ollama always on; Whisper off until Settings `enable_whisper`), `sidecars/addon/` (disable with `BROS_SIDECAR_<ID>=0` or `BROS_SIDECARS_DISABLE`), `sidecars/custom/` (gitignored; Add sidecar and git clones). Runtime data is `$BROS_HOME/data/<id>/` mirroring container paths.
